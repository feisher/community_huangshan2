/**
 */
package com.yusong.community.utils.bga;
...
/**
 * 描述:下拉刷新、上拉加载更多、可添加自定义（固定、可滑动）头部控件
 */
public class BGARefreshLayout extends AutoLinearLayout {
    ...
    /**
     * TODO 添加  如果嵌套recyclerView必须设置数据源和Layoutmanager
     */
    public void setLinearLayoutManager(List data, LinearLayoutManager manager) {
        this.data = data;
     ...
     * TODO 添加  如果嵌套recyclerView必须设置数据源和Layoutmanager
     */
    public void setGridLayoutManager(List data, GridLayoutManager manager) {
        this.data = data;
        ...
        if (mRecyclerView != null) {
            mRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
                ...
                public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                    if (newState == RecyclerView.SCROLL_STATE_IDLE && shouldHandleRecyclerViewLoadingMore(mRecyclerView, UpOrDown)) {
                        if (data != null) {
                            /**
                             * TODO 修改  判断recyclerView是最后一条目显示再加载更多。
                             */
                            if (mLinearLayoutManager != null)
                                if (mLinearLayoutManager.findLastVisibleItemPosition() == data.size() - 1) {
                                    beginLoadingMore();
                                }

                            if (mGridLayoutManager != null) {
                                if (mGridLayoutManager.findLastVisibleItemPosition() == data.size() - 1) {
                                    beginLoadingMore();
                                }
                            }
    ...
    public boolean shouldHandleRecyclerViewLoadingMore(RecyclerView recyclerView, int UpOrDown) {
        ...
        return BGARefreshScrollingUtil.isRecyclerViewToBottom(recyclerView, UpOrDown);
    }
    ...
    private boolean shouldHandleLoadingMore() {
        ...
        if (mRecyclerView != null) {
            ...
            if (data != null) {
                if (mLinearLayoutManager != null)
                    if (mLinearLayoutManager.findLastVisibleItemPosition() == data.size() - 1 && UpOrDown > 0) {
                        return true;
                    } else {
                        return false;
                    }
                ...
                if (mGridLayoutManager != null)
                    if (mGridLayoutManager.findLastVisibleItemPosition() == data.size() - 1 && UpOrDown > 0) {
                        return true;
                    } else {
                        return false;
                    }
        ...
        if (null != mRefreshHeaderView) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    if (isWholeHeaderViewCompleteInvisible() && isContentViewToTop()) {
                        mRefreshDownY = (int) event.getY();
                        return true;
                    }
                    break;
