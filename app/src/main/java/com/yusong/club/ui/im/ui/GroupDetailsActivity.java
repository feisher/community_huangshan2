...
public class GroupDetailsActivity extends IMBaseActivity implements OnClickListener {
                            ...
                            public void run() {
                                try {
                                    ...
                                    runOnUiThread(new Runnable() {
                                        public void run() {
                                            mTitleBar.setTitle(returnData + "(" + group.getMemberCount()
                                                    + st);
                ...
                public void run() {
                    try {
                        ...
                        runOnUiThread(new Runnable() {
                            public void run() {
                                progressDialog.dismiss();
                                ToastUtils.showShort(getApplicationContext(), "屏蔽失败，权限不足！");
                            }
                        });
                    }
    ...
    private class GridAdapter extends ArrayAdapter<String> {
        ...
        public View getView(final int position, View convertView, final ViewGroup parent) {
            ...
            if (position == getCount() - 1) {
                ...
                // 设置成删除按钮
                holder.imageView.setImageResource(R.drawable.em_smiley_minus_btn_pressed);
//				button.setCompoundDrawablesWithIntrinsicBounds(0, R.drawable.smiley_minus_btn, 0, 0);
                ...
                holder.textView.setVisibility(View.GONE);
                holder.imageView.setImageResource(R.drawable.em_smiley_add_btn_pressed);
        ...
        new Thread(new Runnable() {
            public void run() {
                try {
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            mTitleBar.setTitle(group.getGroupName() + "(" + group.getMemberCount()
                                    + ")");
                            refreshMembers();
                            if (EMClient.getInstance().getCurrentUser().equals(group.getOwner())) {
                                // 显示解散按钮
