...
public class EaseNotifier {
    ...
    protected void sendNotification(EMMessage message, boolean isForeground) {
        sendNotification(message, isForeground, false);
    }

    String customNotifyText;
        ...
        try {
            ...
            if (notificationInfoProvider != null) {
                if (message.getType() == EMMessage.Type.CMD) {
                    customNotifyText = notificationInfoProvider.getLatestText(message, fromUsersNum, notificationNum);
                    if (customNotifyText != null) {
                        MsgBean msgBean = JsonUtils.deserialize(customNotifyText, MsgBean.class);
                        Notify date = msgBean.getData();
                        String content = date.getContent();
                        notifyText = content;
                    }
                } else {
                    customNotifyText = notificationInfoProvider.getDisplayedText(message);
                    if (customNotifyText != null) {
